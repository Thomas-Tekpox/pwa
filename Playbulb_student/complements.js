...
// valeur aléatoire entière comprise entre 0 et max (probablement exclu ?!)
function getRandomInt(max) {
  return Math.floor(Math.random() * Math.floor(max));
}

// genere et renvoie une couleur aléatoire au format HTML 
// preferez la fonction suivante getRandomSaturatedColor()
...
// genere et renvoie une couleur aléatoire mais fortement saturée, au format HTML
...
// retourne (au format HTML) la couleur moyenne calculée sur 1/5 de l'image au centre
...
// retourne (au format HTML) la couleur moyenne calculée sur 1/5 de l'image au centre
// en augmentant la saturation.
// id : id du canvas contenant l'image
// blockSize : le pas utilisé pour passer d'un pixel à l'autre,
//  si blockSize = 1 on utilise tous les pixels, si la valeur est
//  supérieure, on prend moins de pixels en compte et le calcul est plus rapide
function couleurDominanteImage2(canvasid, blockSize) {
    var i = -4,
        r = 0,
        g = 0,
        b = 0,
...
// retourne (au format HTML) la couleur moyenne calculée sur 1/5 de l'image au centre
// en augmentant la saturation.
// canvas : C'est l'OBJET canvas qui est passé en parametre
// blockSize : le pas utilisé pour passer d'un pixel à l'autre,
//  si blockSize = 1 on utilise tous les pixels, si la valeur est
//  supérieure, on prend moins de pixels en compte et le calcul est plus rapide
function couleurDominanteImage3(canvas, blockSize) {
    var i = -4,
        r = 0,
        g = 0,
        b = 0,
    ...
    // boucle pour les sommes. En ajustant blockSize, on peut considerer 1 pixel parmi N
    // si blocksize = 1, tous les pixels sont considéré.
    // le canal alpha qui n'est pas pris en considération.
    while ((i += blockSize * 4) < length) {
        ++count;
        fr += pix[i];
        fg += pix[i + 1];
